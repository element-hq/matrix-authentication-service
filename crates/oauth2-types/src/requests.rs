...
pub struct TokenExchangeGrant {
    ...
    /// Identifies which upstream provider's token to return.
    ///
    /// Note: this is a MAS-specific use of the RFC 8693 `audience` parameter.
    /// Rather than naming the target service for the *issued* token, MAS uses
    /// it to select the linked upstream provider (by issuer URL or ULID)
    /// whose stored token should be returned.
...
impl AccessTokenResponse {
    ...
    /// Adds an issued token type to an `AccessTokenResponse`.
    #[must_use]
    pub fn with_issued_token_type(mut self, issued_token_type: String) -> Self {
        self.issued_token_type = Some(issued_token_type);
        self
    }
}
