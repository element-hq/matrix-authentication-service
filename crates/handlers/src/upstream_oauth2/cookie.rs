...
pub struct Payload {
    ...
    /// The username typed on the registration page, when the flow started
    /// there with the field filled in
...
impl UpstreamSessions {
    ...
    pub fn lookup_link(
        &self,
        link_id: Ulid,
    ) -> Result<(Ulid, Option<&PostAuthAction>, Option<&str>), UpstreamSessionNotFound> {
        self.0
            .iter()
...
mod tests {
    ...
    fn test_session_cookie() {
        ...
        // But it can be looked up by its link, along with the username it carries
        let (session, _post_auth_action, username) = sessions.lookup_link(second_link).unwrap();
        assert_eq!(session, second_session);
        assert_eq!(username, Some("john"));
    ...
    /// A cookie payload written before the username field existed still
    /// deserializes, and carries no username
    #[test]
    fn test_payload_without_username() {
        ...
        .expect("payload without a username should deserialize");

        let link = "01FSHN9AG09NMZYX8MFVH74RP4".parse().unwrap();
        let (_session, _post_auth_action, username) = sessions.lookup_link(link).unwrap();
