...
async fn token_exchange_grant(
    ...
    if link_token.is_expired(clock.now()) && link_token.has_refresh_token() {
        ...
        // A missing scope in the response means it is unchanged, per RFC 6749 §6
        let new_scope = token_response
            .scope
            .as_ref()
    ...
    info!(
        ...
        "Released an upstream access token through token exchange"
    );

    // 10. Build the response
    let mut response = AccessTokenResponse::new(upstream_access_token)
        .with_issued_token_type(ACCESS_TOKEN_TYPE_URN.to_owned());
...
mod tests {
    ...
    async fn test_token_exchange_grant(pool: PgPool) {
        ...
        // 6. Without the client being allow-listed, the policy denies the exchange
        let request =
            Request::post(mas_router::OAuth2TokenEndpoint::PATH).form(serde_json::json!({
        ...
        // 7. Allow-list the client and the exchange succeeds
        state.policy_factory = crate::test_utils::policy_factory(
            mas_policy::BaseData {
                server_name: "example.com".to_owned(),
                session_limit: None,
            },
            serde_json::json!({ "token_exchange_clients": [client_id] }),
        )
        .await
        .unwrap();
        ...
        // 8. Test with issuer as audience (should also work)
        let request =
            Request::post(mas_router::OAuth2TokenEndpoint::PATH).form(serde_json::json!({
                "grant_type": "urn:ietf:params:oauth:grant-type:token-exchange",
                "subject_token": subject_token_str,
    ...
    /// A client that didn't register the token exchange grant type can't use it
    #[sqlx::test(migrator = "mas_storage_pg::MIGRATOR")]
    async fn test_token_exchange_unregistered_grant(pool: PgPool) {
        setup();
        let state = TestState::from_pool(pool).await.unwrap();

        // Register a client *without* the token exchange grant type
        let request =
            Request::post(mas_router::OAuth2RegistrationEndpoint::PATH).json(serde_json::json!({
                "client_uri": "https://example.com/",
                "token_endpoint_auth_method": "none",
        ...
        // The grant type check happens before subject token validation, so a dummy
        // subject token is enough to exercise it.
        let request =
            Request::post(mas_router::OAuth2TokenEndpoint::PATH).form(serde_json::json!({
                "grant_type": "urn:ietf:params:oauth:grant-type:token-exchange",
                "subject_token": "mat_unused",
                "subject_token_type": "urn:ietf:params:oauth:token-type:access_token",
                "audience": "https://upstream.example.com/",
                "client_id": client_id,
            }));
        ...
        let ClientError { error, .. } = response.json();
        assert_eq!(error, ClientErrorCode::UnauthorizedClient);
    }
}
