...
mod tests {
    ...
    async fn test_query_mode_interstitial_redirect(pool: PgPool) {
        ...
        // The page bounces to the callback URL carrying the error, via the
        // `redirect.ts` entrypoint...
        assert!(body.contains("/assets/redirect-"));
        // The entrypoint reads the target off this link, so the hook has to match
        assert!(body.contains(r#"id="redirect-link""#));
    ...
    /// interstitial).
    /// `prompt=none` runs in a hidden iframe, which cannot render the
    /// interstitial, so it must be checked before the other parameter errors —
    /// they would each hand back an interstitial instead of a redirect
    #[sqlx::test(migrator = "mas_storage_pg::MIGRATOR")]
    async fn test_prompt_none_wins_over_other_parameter_errors(pool: PgPool) {
        setup();
        let state = TestState::from_pool(pool).await.unwrap();
        let client_id = register_client(&state).await;
