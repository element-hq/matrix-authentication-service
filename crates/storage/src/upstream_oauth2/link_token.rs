...
pub trait UpstreamOAuthLinkTokenRepository: Send + Sync {
    ...
    /// * `scope`: The scope of the new token, if known
    ///
    /// A `None` `encrypted_refresh_token` or `scope` leaves the stored value
    /// untouched, as providers may omit both when refreshing
    ///
    /// # Errors
    ///
    /// Returns [`Self::Error`] if the underlying repository fails
