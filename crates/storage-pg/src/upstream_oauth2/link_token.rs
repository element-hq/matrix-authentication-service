...
use mas_data_model::{Clock, UlidExt as _, UpstreamOAuthLink, UpstreamOAuthLinkToken};
...
impl UpstreamOAuthLinkTokenRepository for PgUpstreamOAuthLinkTokenRepository<'_> {
    ...
    async fn update_tokens(
        ...
        // Providers may omit the refresh token and the scope when refreshing, in which
        // case the previous values still apply
        let res = sqlx::query!(
            r#"
        ...
        Ok(UpstreamOAuthLinkToken {
            id: link_token.id,
            link_id: link_token.link_id,
            encrypted_access_token,
            encrypted_refresh_token: encrypted_refresh_token.or(link_token.encrypted_refresh_token),
            access_token_expires_at: expires_at,
            token_scope: scope.or(link_token.token_scope),
            created_at: link_token.created_at,
            updated_at,
        })
    }
}
