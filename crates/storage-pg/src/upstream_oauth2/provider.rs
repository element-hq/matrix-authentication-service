...
impl UpstreamOAuthProviderRepository for PgUpstreamOAuthProviderRepository<'_> {
    ...
    async fn find_by_issuer(
        ...
        // The issuer isn't unique, so we can't tell which provider was meant if
        // multiple enabled ones share it
        if res.len() > 1 {
            return Err(DatabaseInconsistencyError::on("upstream_oauth_providers")
                .column("issuer")
                .into());
        }
