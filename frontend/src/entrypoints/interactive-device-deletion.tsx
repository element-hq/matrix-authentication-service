...
import { Button, H3, TooltipProvider } from "@vector-im/compound-web";
...
const el = document.getElementById(
  "interactive-device-deletion",
) as HTMLElement;
// TODO const data = v.parse(schema, (window as any).RENDER_DATA);
...
function Sessions(): React.ReactElement {
  ...
  const [{ inactive, pagination }, setParams] = useState({
    inactive: undefined,
    pagination: { first: 50 },
  });
  const {
      ...
      {(forwardPage || backwardPage || true) && (
        <div className="flex *:flex-1">
          <Button
            ...
            onClick={() =>
              setParams({ inactive, pagination: forwardPage || pagination })
            }
            resetScroll
          >
            {t("common.previous")}
          ...
          <Button
            ...
            onClick={() =>
              setParams({ inactive, pagination: backwardPage || pagination })
            }
            resetScroll
          >
            {t("common.next")}
